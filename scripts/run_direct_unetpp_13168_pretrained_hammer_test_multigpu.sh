...
CHECKPOINT="${CHECKPOINT:-${ROOT_DIR}/checkpoints_direct_unetpp_13168_pretrained_hammer/best_val.pth}"
OUTPUT_DIR="${OUTPUT_DIR:-${ROOT_DIR}/direct_unetpp_13168_pretrained_hammer_test_outputs}"
...
NUM_SHARDS="${#GPUS[@]}"
echo "[launch] pretrained Direct U-Net++ HAMMER test: ${NUM_SHARDS} shards on ${GPU_IDS}"
...
for SHARD_INDEX in "${!GPUS[@]}"; do
  ...
  (
    CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" infer_direct_unetpp_hammer.py \
...
echo "[done] pretrained Direct U-Net++ HAMMER test complete"
