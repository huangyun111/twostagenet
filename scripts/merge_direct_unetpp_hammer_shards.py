"""Merge sharded Direct U-Net++ HAMMER metrics and verify unique coverage."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Merge Direct U-Net++ HAMMER shards.")
    parser.add_argument("--output_dir", type=Path, required=True)
    parser.add_argument("--expected_count", type=int, required=True)
    return parser.parse_args()


def read_shards(output_dir: Path) -> tuple[list[dict[str, str]], list[dict[str, object]]]:
    ...
    for shard_dir in sorted(output_dir.glob("shard_*")):
        ...
        with metrics_path.open(newline="", encoding="utf-8") as handle:
            rows.extend(row for row in csv.DictReader(handle) if row["name"] != "Overall")
        summaries.append(json.loads(summary_path.read_text(encoding="utf-8")))
    rows.sort(key=lambda row: row["name"])
    if not rows or not summaries:
        raise RuntimeError(f"No complete shards found under {output_dir}")
...
def main() -> None:
    args = parse_args()
    rows, shard_summaries = read_shards(args.output_dir)
    ...
    payload = {
        ...
        "output_resized_to_gt_count": sum(
            int(summary.get("output_resized_to_gt_count", 0)) for summary in shard_summaries
        ),
