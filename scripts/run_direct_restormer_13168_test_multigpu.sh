...
CHECKPOINT="${CHECKPOINT:-${ROOT_DIR}/checkpoints_direct_restormer_13168/best_val.pth}"
OUTPUT_DIR="${OUTPUT_DIR:-${ROOT_DIR}/direct_restormer_13168_test_outputs}"
...
NUM_SHARDS="${#GPUS[@]}"
echo "[launch] Direct Restormer test inference with ${NUM_SHARDS} shards: ${GPU_IDS}"
...
for SHARD_INDEX in "${!GPUS[@]}"; do
  ...
  (
    CUDA_VISIBLE_DEVICES="${GPU_ID}" "${PYTHON_BIN}" infer_direct_restormer_baseline.py \
      --manifest "${MANIFEST}" \
...
"${PYTHON_BIN}" scripts/merge_direct_unetpp_shards.py \
  ...
  --expected_count 1474
echo "[done] Direct Restormer test inference complete"
