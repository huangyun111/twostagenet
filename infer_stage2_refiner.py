"""Run Stage 2 confidence-guided residual refiner inference and evaluation."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Infer and evaluate Stage 2 confidence-guided residual refiner."
    )
...
def save_visualization(
    ...
    residual_map = residual.abs().mean(dim=0)
    ...
    panels = [
        ...
        ("Gate DoLP", gate[0].detach().cpu().numpy(), (0.0, 1.0)),
        ("Mean gate", mean_gate.detach().cpu().numpy(), (0.0, 1.0)),
        ("Mean abs residual", residual_map.detach().cpu().numpy(), (0.0, 0.3)),
