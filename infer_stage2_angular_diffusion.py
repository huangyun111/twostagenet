"""Run Stage 2 angular residual diffusion inference and evaluation."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Infer and evaluate Stage 2 angular diffusion.")
    ...
    parser.add_argument("--output_dir", type=str, default="./stage2_angular_diffusion_outputs")
...
def save_visualization(
    ...
    delta_angle_deg = pred["pred_delta_angle"][0] * (180.0 / math.pi)
    ...
    panels = [
        ...
        ("Diffusion refined DoLP", refined[0].detach().cpu().numpy(), (0.0, 1.0)),
        ...
        ("Diffusion AoLP", aolp_to_display(polar_to_aolp(refined)), (0.0, 1.0)),
        ...
        ("Delta DoLP", pred["pred_delta_dolp"][0].detach().cpu().numpy(), (-0.3, 0.3)),
    ]
...
def write_summary(
    ...
    lines = [
        "Stage 2 angular residual diffusion test summary",
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            for offset, name in enumerate(names):
                ...
                np.save(refined_dir / f"{name}.npy", sample_refined.numpy().astype(np.float32))
