"""Infer and evaluate Direct U-Net++ on a HAMMER folder split."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Infer Direct U-Net++ on HAMMER.")
    parser.add_argument("--root_dir", type=str, required=True)
    parser.add_argument("--checkpoint", type=str, required=True)
    parser.add_argument("--output_dir", type=str, required=True)
    parser.add_argument("--preprocess_mode", choices=("resize256", "official_infer"), default="official_infer")
...
def load_model(args: argparse.Namespace, device: torch.device) -> DirectUnetPlusPlusBaseline:
    model = DirectUnetPlusPlusBaseline(encoder_name=args.encoder_name)
...
def main() -> None:
    ...
    payload = {
        "model_type": "direct_unetpp_13168_pretrained_hammer_finetuned",
        "samples": len(rows),
