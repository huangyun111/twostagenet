"""Loss for the Version E asymmetric Restormer refiner."""
...
class Stage2AsymmetricRestormerLoss(nn.Module):
    """Balance scalar DoLP correction with Version D's circular AoLP objective."""
    ...
    def __init__(
        ...
        lambda_dolp_residual: float = 0.5,
        lambda_vector: float = 1.0,
        lambda_aolp: float = 2.0,
        lambda_high_dolp_aolp: float = 1.0,
        lambda_edge: float = 0.2,
        lambda_angle_residual_reg: float = 0.02,
        lambda_angle_gate_reg: float = 0.01,
        lambda_angle_no_harm: float = 0.2,
        ...
        self.lambda_dolp_residual = lambda_dolp_residual
        self.lambda_vector = lambda_vector
        self.lambda_aolp = lambda_aolp
        self.lambda_high_dolp_aolp = lambda_high_dolp_aolp
        self.lambda_edge = lambda_edge
        self.lambda_angle_residual_reg = lambda_angle_residual_reg
        self.lambda_angle_gate_reg = lambda_angle_gate_reg
        self.lambda_angle_no_harm = lambda_angle_no_harm
    ...
    def forward(
        ...
        normalized_dolp_error = (
            refined[:, 0:1] - target[:, 0:1]
        ) / self.dolp_residual_scale
        loss_dolp = self._charbonnier(normalized_dolp_error).mean()

        target_delta_dolp = (target[:, 0:1] - prior[:, 0:1]).clamp(
            -self.dolp_residual_scale, self.dolp_residual_scale
        )
        normalized_delta_error = (
            pred["delta_dolp"] - target_delta_dolp
        ) / self.dolp_residual_scale
        loss_dolp_residual = self._charbonnier(normalized_delta_error).mean()

        vector_error = F.smooth_l1_loss(
            refined[:, 1:3], target[:, 1:3], reduction="none"
        )
        ...
        normalized_delta_angle = pred["delta_angle"].abs() / self.angle_residual_scale
        loss_residual_reg = normalized_delta_angle.mean()
        confidence_angle = confidence[:, 1:3].mean(dim=1, keepdim=True).clamp(0.0, 1.0)
        loss_gate_reg = self._weighted_mean(
            pred["gate_angle"] * confidence_angle * normalized_delta_angle,
            reliability,
        )

        prior_aolp_error = self._aolp_error_rad(prior, target)
        loss_no_harm = self._weighted_mean(
            F.relu(aolp_error - prior_aolp_error), reliability
        )

        total = (
            self.lambda_dolp * loss_dolp
            + self.lambda_dolp_residual * loss_dolp_residual
            + self.lambda_vector * loss_vector
            + self.lambda_aolp * loss_aolp
            + self.lambda_high_dolp_aolp * loss_high_dolp_aolp
            + self.lambda_edge * loss_edge
            + self.lambda_angle_residual_reg * loss_residual_reg
            + self.lambda_angle_gate_reg * loss_gate_reg
            + self.lambda_angle_no_harm * loss_no_harm
        )

        return {
            "loss": total,
            "loss_dolp": loss_dolp.detach(),
            "loss_dolp_residual": loss_dolp_residual.detach(),
    ...
    def _charbonnier(self, values: torch.Tensor) -> torch.Tensor:
        return torch.sqrt(values.square() + self.charbonnier_eps**2) - self.charbonnier_eps
    ...
    def _edge_loss(
        ...
        angle_edge = self._weighted_mean(
            (refined_dx[:, 1:3] - target_dx[:, 1:3]).abs(), reliability_dx
        )
        angle_edge = angle_edge + self._weighted_mean(
            (refined_dy[:, 1:3] - target_dy[:, 1:3]).abs(), reliability_dy
        )
